...
app.post("/login", (req, res) => {
  ...
  const doctorSql = `SELECT * FROM doctors WHERE username = ? AND password_hash = ?`;
  con.query(doctorSql, [Lusername, Lpassword], (doctorErr, doctorResult) => {
    if (doctorErr) throw doctorErr;

    if (doctorResult.length > 0) {
      res.status(200).redirect("/doctors");
    } else {
      var sql = `SELECT * FROM hospitals WHERE username = ? AND password_hash = ?`;
...
app.post("/showP", (req, res) => {
  ...
  var sql = `SELECT * FROM patient_information WHERE patient_id = ?`;
  con.query(sql, patientID, (err, result) => {
    ...
    if (result.length > 0) {
      ...
      res.json({ error: "There is no such a user in the database" });
    }
  });
});
